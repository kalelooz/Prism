...
const paths = await packager({ dir: '.', name: 'Prism', platform: 'win32', arch: 'x64',
  ...
  ignore: [/^\/(dist[^/]*|artifacts|work|plans|publish|docs|src-tauri|tests|node_modules|\.git|\.github)(\/|$)/, /\.test\.mjs$/, /\.lnk$/i, /\/(CODEX_PROJECT\.md|CONTEXT\.md|PRO_PLAN\.md|READABILITY\.md|RELEASE_SETUP\.md|VERIFICATION\.md|prism-feasibility-[^/]+\.md|\.c2cignore|\.gitignore|native-(assets|entry|run)\.mjs|package\.mjs|preview\.cjs|wallpaper-restart\.(ps1|log))$/, /(^|\/)\.env(\.[^/]*)?$/, /\.(pfx|p12|key|pem)$/i],
